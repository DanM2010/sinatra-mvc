...
describe "Our Person Create Route" do
  ...
  it 'creates a person when we post to /people with a birthdate in the form %d%m%Y' do
    people_count = Person.all.count
    post("/people", { first_name: @person.first_name, last_name: @person.last_name, birthdate: @person.birthdate.strftime("%d%m%Y") })
    expect(Person.all.count).to eq(people_count + 1)
  end  
